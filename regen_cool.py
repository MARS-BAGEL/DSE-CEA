...
fuel_pressure = 1500000 #[Pa] - pressure of fuel in coolant channel (assumed to be constant)
...
def get_temp_changes(T_left, T_right, T_fuel):
    ...
    c_fuel = gas_fluid_properties.get_fuel_properties(T_fuel, fuel_pressure, O2, stage)[0] #need specific heat of fuel to work out temp rise
...
def simulate():
    ...
    while time < burn_time:
        ...
        if counter%300 == 0:
            print(f"time = {time}")
            print(f"max fluid temp = {np.max(T_f)}")
            print(f"max hot wall temp = {np.max(T_L)}")
